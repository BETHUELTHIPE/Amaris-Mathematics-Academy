    ...
    <section className="mx-auto grid max-w-7xl gap-10 px-5 py-12 lg:grid-cols-[.8fr_1.2fr] lg:items-start lg:px-8 lg:py-20">
      ...
      <div className="rounded-[2rem] border border-[#dce4ef] bg-white p-6 shadow-[0_25px_75px_rgba(7,21,45,.1)] sm:p-9">
        ...
        <form action={registerAction} className="mt-7 grid gap-5">
          ...
          <AuthField label="School, college, university or organisation" name="institution" autoComplete="organization" />
          ...
          <p className="-mt-2 text-xs leading-5 text-[#68778f]">Use 12–72 characters with uppercase, lowercase, a number and a symbol.</p>
          ...
          <button type="submit" className="min-h-12 rounded-full bg-[#0b2a5b] px-6 py-3 font-bold text-white transition hover:bg-[#123d79]">Create account</button>
        </form>
        <p className="mt-6 text-center text-sm text-[#69778f]">Already registered? <Link href="/login" className="font-bold text-[#1f5bbd]">Log in</Link></p>
      </div>
    </section>
    <Footer />
  </main>;
