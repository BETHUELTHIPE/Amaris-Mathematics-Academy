...
import { initialEnquiryState } from "@/app/contact/state";
...
function SubmitButton() {
  ...
  return <Button type="submit" disabled={pending} className="h-12 rounded-full bg-[#0b2a5b] px-7 text-base font-bold text-white hover:bg-[#133b76]">{pending ? <><LoaderCircle className="size-4 animate-spin" />Sending enquiry securely…</> : <>Send enquiry <Send className="size-4" /></>}</Button>;
}
...
export function EnquiryForm() {
  ...
  return (
    <form action={formAction} className="mt-8 space-y-6">
      ...
      <div><div className="flex items-start gap-3 text-sm leading-6 text-[#60708a]"><input id="consent" name="consent" type="checkbox" required value="on" aria-labelledby="consent-label" aria-invalid={Boolean(state.errors?.consent)} className="mt-0.5 size-6 shrink-0 cursor-pointer rounded border border-[#9aabc0] accent-[#1f5bbd]" /><span id="consent-label">I agree that Amaris Mathematics Academy may use these details, including AI-assisted support, to respond to my enquiry. <span className="text-[#b42318]">*</span></span></div><FieldError message={state.errors?.consent} /></div>
