  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#07152d]/95 text-white backdrop-blur-xl">
      ...
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
        <Link href="/" prefetch={false} className="relative z-10 flex shrink-0 items-center gap-3 font-semibold tracking-tight" aria-label="Amaris Mathematics Academy home">
        ...
        <nav className="hidden items-center gap-6 text-sm text-white/75 lg:flex" aria-label="Main navigation">
          {links.map(({ label, url, open_in_new_tab }) => <Link key={url} href={url} prefetch={false} target={open_in_new_tab ? "_blank" : undefined} rel={open_in_new_tab ? "noreferrer" : undefined} className="transition hover:text-white">{label}</Link>)}
        </nav>
