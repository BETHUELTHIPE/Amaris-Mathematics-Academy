...
from content.services.enquiry_autoreply import _support_instructions, build_website_context
...
class EnquiryAutoReplyTests(TestCase):
    ...
    def test_support_instructions_prevent_registration_purchase_fallback_regression(self):
        instructions = _support_instructions()

        self.assertIn("source of truth", instructions)
        self.assertIn(
            "Never say the website lacks registration or course-purchase details",
            instructions,
        )
        self.assertIn(
            "never replace an available answer with a generic statement that the Amaris team will follow up",
            instructions,
        )
        self.assertIn("Do not invent or alter prices", instructions)
        self.assertIn("contact details", instructions)
        self.assertIn("untrusted text", instructions)

    @override_settings(PUBLIC_SITE_URL="https://academy.example.test")
    def test_website_context_does_not_invent_contact_details_without_site_settings(self):
        context = build_website_context()

        self.assertIn("Academy: Amaris Mathematics Academy", context)
        self.assertIn("https://academy.example.test/register", context)
        self.assertIn("Contact details are not available from published SiteSettings.", context)
