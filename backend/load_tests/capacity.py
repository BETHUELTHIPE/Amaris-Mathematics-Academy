...
def _reject_weaker_threshold(name: str, value: float, maximum_allowed: float) -> float:
    if value > maximum_allowed:
        raise ValueError(
            f"{name}={value} would weaken the capacity gate; it must be <= {maximum_allowed}."
        )
    return value
...
class CapacityThresholds:
    ...
    def from_environment(cls) -> CapacityThresholds:
        ...
        server_5xx_pct = env_float("CAPACITY_MAX_5XX_PCT", MAX_SERVER_5XX_PCT)
        ...
        infrastructure_cpu_pct = env_float(
            "CAPACITY_MAX_CPU_PCT", MAX_INFRASTRUCTURE_CPU_PCT
        )
        infrastructure_ram_pct = env_float(
            "CAPACITY_MAX_RAM_PCT", MAX_INFRASTRUCTURE_RAM_PCT
        )

        _reject_weaker_threshold("CAPACITY_MAX_FAILURE_PCT", failure_pct, MAX_FAILURE_PCT)
        _reject_weaker_threshold(
            "CAPACITY_MAX_5XX_PCT", server_5xx_pct, MAX_SERVER_5XX_PCT
        )
        _reject_weaker_threshold("CAPACITY_MAX_P95_MS", float(p95_ms), float(MAX_P95_MS))
        _reject_weaker_threshold("CAPACITY_MAX_P99_MS", float(p99_ms), float(MAX_P99_MS))
        _reject_weaker_threshold(
            "CAPACITY_MAX_CPU_PCT", infrastructure_cpu_pct, MAX_INFRASTRUCTURE_CPU_PCT
        )
        _reject_weaker_threshold(
            "CAPACITY_MAX_RAM_PCT", infrastructure_ram_pct, MAX_INFRASTRUCTURE_RAM_PCT
        )
