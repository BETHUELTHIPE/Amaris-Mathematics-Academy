...
RESULT_PATH = Path(os.getenv("CAPACITY_LOCUST_RESULT", "load_tests/results/capacity-locust.json"))
...
class CapacityStudentUser(HttpUser):
    ...
    def dashboard(self) -> None:
        _checked_get(self, ENDPOINTS.dashboard, "07 Student dashboard", protected=True)

    @task(6)
    def lesson_access(self) -> None:
        _checked_get(self, ENDPOINTS.lesson, "08 Lesson access", protected=True)

    @task(3)
    def payment_status_polling(self) -> None:
        _checked_get(self, ENDPOINTS.payment_status, "11 Payment-status polling", protected=True)
...
def report_capacity_counters(client_id: str, data: dict[str, Any], **_kwargs: Any) -> None:
    del client_id
...
def receive_capacity_counters(client_id: str, data: dict[str, Any], **_kwargs: Any) -> None:
...
def validate_capacity_run(environment: Environment, **_kwargs: Any) -> None:
    ...
    validate_target(
        target_url,
        environment_name=os.getenv("LOADTEST_ENVIRONMENT", "capacity").strip().lower(),
    ...
    if REQUIRE_AUTH:
        ...
        if missing:
            raise RuntimeError(f"Representative capacity testing requires: {', '.join(missing)}")
...
def write_capacity_evidence(environment: Environment, **_kwargs: Any) -> None:
    ...
    if server_5xx_pct > threshold.server_5xx_pct:
        failures.append(f"5xx rate {server_5xx_pct:.3f}% exceeded {threshold.server_5xx_pct:.3f}%")
    if evidence["p95_ms"] > threshold.p95_ms:
        failures.append(f"p95 {evidence['p95_ms']}ms exceeded {threshold.p95_ms}ms")
    if evidence["p99_ms"] > threshold.p99_ms:
        failures.append(f"p99 {evidence['p99_ms']}ms exceeded {threshold.p99_ms}ms")
    ...
    if failures:
        ...
        LOGGER.info("Capacity Locust gate passed at %s concurrent users.", f"{TARGET_USERS:,}")
